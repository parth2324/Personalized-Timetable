...
class Scheduler
{
	...
	{
		final int favorableTimes[]={12}, priorities[]={0, 1};
		final String[] courses={"csc148S", "mat137Y"};
...
class Schedule
{
	...
	public boolean getCourseData()
	{
		for(int i=0;i<crcs.length;i++)
		{
			try
		    {
                str=new URL("https://timetable.iit.artsci.utoronto.ca/api/20229/courses?code="+crcs[i].substring(0,crcs[i].length()-1)+"&section="+Character.toUpperCase(crcs[i].charAt(crcs[i].length()-1))).openConnection().getInputStream();  
                tmp="";
