...
class Scheduler:
    ...
    # TODO: add greedy mode (estimated score range based), stabalize scoring system
    data: Dict
    _initialized: bool
    ...
    def __init__(self, crcs: List[str], year: int = time.strptime(time.ctime()).tm_year) -> None:
        self.data = {ID: []}
        ...
        i = 0
        for crc in crcs:
            workers.append(threading.Thread(target=self._read_data, args=(crc, year, i), daemon=True))
            i += 1
    ...
    def _read_data(self, crc: str, year: int, id: int) -> None:
    ...
    def _add_course(self, crc: str, state: Tuple[Tuple, Tuple, List], lec_to_ignore: List[int], tut_to_ignore: List[int], do_lec: bool = True, do_tut: bool = True) -> List:    
        ...
        if winter != (F in self.data[crc]):
            ...
            if do_lec:
                ...
                new_state = self._try_push_winter(lec_fall[1], lec_fall[0], tut_to_ignore + NEW_STATE_ITERATOR_PLACE_HOLDER, [], to_ignore[:])
            ...
            if do_tut:
                ...
                new_state_ = self._try_push_winter(tut_fall[1], tut_fall[0], to_ignore + NEW_STATE_ITERATOR_PLACE_HOLDER, [], lec_to_ignore[:])
    ...
    def _state_iterator_fall(self, addr: int, state: Tuple[Tuple, Tuple, List], args: Optional[Tuple], lec_to_ignore: List[int], tut_to_ignore: List[int]) -> List:
        comm = [True, False, None, None, None] # [needed, ready, clone]
        def init_cloner(comm: List, state: Tuple[Tuple, Tuple, List], lec_to_ignore: List[int], tut_to_ignore: List[int]) -> None:
            while comm[0]:
                if comm[1]:
                    time.sleep(SLEEP)
                    continue
                ...
                comm[4] = tut_to_ignore + NEW_STATE_ITERATOR_PLACE_HOLDER
                comm[1] = True
        threading.Thread(target=init_cloner, args=(comm, state, lec_to_ignore, tut_to_ignore), daemon=True).start()
        workers = []
        ...
        for lec in dict[ID]:
            ...
            while not comm[1]:
                time.sleep(SLEEP)
            workers.append(Concurrent_Runner(self._try_push_fall, (lec_addr, comm[2], comm[4], [], comm[3], 1), lec_addr))
            comm[1] = False
            workers[i].start()
            i += 1
        comm[0] = False
    ...
    def _state_iterator_winter(self, addr: int, state: Tuple[Tuple, Tuple, List], args: Optional[Tuple], lec_to_ignore: List[int], tut_to_ignore: List[int]) -> List:
        ...
        for lec in dict[ID]:
            ...
            while not comm[1]:
                time.sleep(SLEEP)
            workers.append(Concurrent_Runner(self._try_push_winter, (lec_addr, comm[2], comm[4], [], comm[3], 1), lec_addr))
            comm[1] = False
            workers[i].start()
            i += 1
        comm[0] = False
    ...
    # can decrease function calls by direct stage passing.
    def _try_push_fall(self, addr: int, state: Tuple[Tuple, Tuple, List], attempts: List[int], stg2_attempts: List[int], lec_attempts: List[int], stage: int = 1) -> Optional[Tuple[Tuple, Tuple, List]]:
        if ASYNC_MAP_R[(addr & ASYNC_MASK) >> ASYNC_SHFT]:
            ...
            state[2][2] += self._week_weight * abs(self._week_balance_invert - week_score) + self.data[crc][session][type][lec][TSCORE]
            ...
            if min_priority_tut_conflict is None:
                del stg2_attempts
                return self._try_push_fall(addr, state, attempts[0 : attempts.index(None) + 1], [], lec_attempts, 3) # elevation
            ...
            if result[0] is None: # conflicting tutorial removed in process for stage 2
                return self._try_push_fall(addr, state, attempts, stg2_attempts, lec_attempts, 2)
            # if len(attempts) > 0 and (min_priority_tut_conflict & CRC_MASK) != (attempts[len(attempts) - 1] & CRC_MASK):
            #     stg2_attempts = []
            return self._try_push_fall(addr, result[0], attempts, stg2_attempts, lec_attempts, 1)
        elif stage == 2:
            ...
            if result[0] is not None:
                # lec_attempts.append(result[len(result) - 1] & CLEAN_MASK)
                return self._try_push_fall(addr, result[0], attempts, stg2_attempts, lec_attempts, 1)
        return None

    def _try_push_winter(self, addr: int, state: Tuple[Tuple, Tuple, List], attempts: List[int], stg2_attempts: List[int], lec_attempts: List[int], stage: int = 1) -> Optional[Tuple[Tuple, Tuple, List]]:
        if ASYNC_MAP_R[(addr & ASYNC_MASK) >> ASYNC_SHFT]:
            week_load = state[2][1]
            avg = (week_load[0] + week_load[1] + week_load[2] + week_load[3] + week_load[4] + week_load[5] + week_load[6]) / 7.0
            week_score = abs(avg - week_load[0]) + abs(avg - week_load[1]) + abs(avg - week_load[2]) + abs(avg - week_load[3]) + abs(avg - week_load[4]) + abs(avg - week_load[5]) + abs(avg - week_load[6])
            ...
            if min_priority_tut_conflict is None:
                del stg2_attempts
                return self._try_push_winter(addr, state, attempts[0 : attempts.index(None) + 1], [], lec_attempts, 3) # elevation
            attempts.append(min_priority_tut_conflict & CLEAN_MASK)
            self._remove_lecture(min_priority_tut_conflict, state)
            result = self._add_course(self.data[ID][conflict_addr & CRC_MASK], state, lec_attempts, attempts, do_lec=False)
            if result[0] is None: # conflicting tutorial removed in process for stage 2
                return self._try_push_winter(addr, state, attempts, stg2_attempts, lec_attempts, 2)
            # if len(attempts) > 0 and (min_priority_tut_conflict & CRC_MASK) != (attempts[len(attempts) - 1] & CRC_MASK):
            #     stg2_attempts = []
            return self._try_push_winter(addr, result[0], attempts, stg2_attempts, lec_attempts, 1)
        elif stage == 2:
            # assumptions: stage 1 called already, there is an associated lecture for conflicting course tutorial (removed already).
            lec_addr = None
            if stg2_attempts == []: # do conflicting course discovery on first run
    ...
    # can reduce code length by smarter if condition for session.
    ...
    def print_schedule(self, state: Tuple = None) -> None:
        ...
        if lecs != "":
            print("asynchronous lectures and tutorials :", lecs)
        print('fall week weights :', state[2][0])
        print('winter week weights :', state[2][1])
        print('number of fall courses :', state[2][3])
        print('number of winter courses :', state[2][4])
        print('schedule score :', round(state[2][2], PRECISION))
...
if __name__ == '__main__':
    ...
    sch = Scheduler(crcs, 2021)
    timePrefnsFall = [1200]
    timePrefnsWinter = [1200]
    balanced = True
    # weights = [0.2, 0.3] # more to balance
    weights = [3, 2] # more to time
    print("Courses read: ", sch.get_read_courses())
    crcPrefns = ['csc148S', 'mat137Y', 'eco101S', 'eco101F'] # TODO: request read crcs from sch and ask user to order descending priority.
